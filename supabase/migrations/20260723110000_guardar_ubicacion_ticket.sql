-- Sustituye la RPC anterior para guardar el punto GPS del automovilista
-- junto con el texto de referencia que ya proporciona en el formulario.
drop function if exists public.solicitar_ayuda(text, text, text, text);

create function public.solicitar_ayuda(
  p_nombre text,
  p_telefono text,
  p_ubicacion text,
  p_descripcion text,
...
begin
  ...
  values (trim(p_nombre), p_telefono)
