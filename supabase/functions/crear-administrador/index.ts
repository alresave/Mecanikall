...
/** Invita a un nuevo administrador; solo un administrador vigente puede hacerlo. */
Deno.serve(async (request) => {
  ...
  const { error: insertError } = await admin.from('administradores').insert({ id_usuario: data.user.id });
  if (insertError) {
